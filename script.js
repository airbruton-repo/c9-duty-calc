...
function getLimitMessage(totalMins) {
    if (totalMins === 0) {
        ...
        return { text: '→ 16:00 actual max duty limit applies', className: 'text-[10px] text-blue-600 mt-1 text-center font-bold' };
    } else if (totalMins <= 720) {
        return { text: '→ 16:30 actual max duty limit applies', className: 'text-[10px] text-green-600 mt-1 text-center font-bold' };
    } else {
...
function updateMultiSegSummary(totalMins, segments) {
    ...
    let dutyMaxText = '';
    if (totalHours > 8) {
        dutyMaxText = 'Actual Duty Max: 16:30 (Sched FTM 8:01-12:00)';
    } else {
        dutyMaxText = 'Actual Duty Max: 16:00 (Sched FTM ≤ 8:00)';
    }
